...
class TablePriceSellCubit extends SafeCubit<TotalOfferModel> {
  TablePriceSellCubit(this.applyOfferImp)
  ...
  void getTotalPrice() {
    ...
    for (final entry in cartSellHome) {
      ...
      double unitPrice = entry.editedPriceModel.pricesUnitPrice ??
          entry.getProductModel.chosenPriceModel.pricesUnitPrice ??
          0;
      var totalProduct =
          (unitPrice) * (entry.productCount) * (entry.productConversion ?? 1);
