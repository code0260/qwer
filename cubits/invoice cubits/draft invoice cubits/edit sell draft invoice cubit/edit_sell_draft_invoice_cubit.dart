...
part 'edit_sell_draft_invoice_state.dart';

class EditSellDraftInvoiceCubit extends SafeCubit<EditSellDraftInvoiceState> {
  InvoiceModel invoiceModel = const InvoiceModel();
  EditSellDraftInvoiceCubit() : super(EditSellDraftInvoiceInitial());

  Future<void> editSellDraftInvoice() async {
    emit(EditSellDraftInvoiceLoading());
    prepareAmountCurrency();
    Either<Failure, int> result;
    result = await StoreInvoices.editSellInvoiceLocally(invoiceModel);
    result.fold((failure) {
      emit(EditSellDraftInvoiceFailure(errorMessage: failure.errMessage));
    }, (invoiceId) {
      emit(EditSellDraftInvoiceSuccess(invoiceId: invoiceId));
    });
  }
